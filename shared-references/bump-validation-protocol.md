...
**格式立场（v1.5 起）**：bump 只作用于**单一格式**的 rubric——多格式项目中各格式 rubric 独立进化、互不干扰。一次 bump 从头到尾只属于一个 `format`（cheat-bump 在流程开头解析并锁定格式），本协议中"校准池""所有样本"等措辞**一律指该格式的样本子集**。rubric_notes.md / rubric-memo.md 是单文件、始终对应 `default_format` 的 rubric，因此当前只支持 bump 默认格式的 rubric；其他格式要进化 rubric，先把 `default_format` 切过去再走本协议。
...
校准池定义：`predictions/*.md` 中**Format 头等于本次 bump 格式**、且有完整复盘段（含 `actual_*` 数据）的文件。跨格式样本一律排除——三格式的 rubric 维度与指标单位不同，混池重打分会让 Step 3 的排序一致性失去意义。缺 Format 头的 1.5 前老文件按 `video` 处理。
...
2. 用**本格式主指标**给所有样本排序——指标口径按 [state-management.md 实绩指标映射](state-management.md#实绩指标映射按格式)：video=actual_plays、article=actual_reads、xhs=actual_likes+actual_collects
...
1. 调用 `mcp__llm-chat__chat`，把以下打包发给外部 LLM：
   - 旧公式 + 新公式 + 本次 bump 的 format
   - 校准池所有样本的：维度分数、composite（新+旧）、实绩数据（主指标 + 辅助指标，口径按 [state-management.md 实绩指标映射](state-management.md#实绩指标映射按格式)）
   - Step 3 的排序对照表
2. 外部 LLM 独立判定两件事：
   - 排序一致性是否真的 ≥ THRESHOLD？
   - 新公式相比旧公式是否解释力更强？
...
`CROSS_MODEL_AUDIT=false`：跳过外部审核——**仅在离线/无网时使用**。state file 标记 `formats.<fmt>.last_bump_self_audited: true`（旧 schema 写顶层），`cheat-status` 持续提示用户配置外部审核。
...
6. 更新**本格式**校准样本的 prediction 文件**底部**追加一行：`**Re-scored under <new-version> on YYYY-MM-DD**: composite=X.XX → Y.YY`（不动预测段、不动复盘段，只追加这一行）。其他格式的 prediction 文件一个字都不碰
