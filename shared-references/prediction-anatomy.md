...
字段必填规则：
- **`Format` 必填**——`video` / `article` / `xhs`，与 `formats.<format>` 命名空间一一对应。这是校准池按格式过滤的唯一依据：cheat-bump 全量重打分、bucket 重校只取同格式样本，没有 Format 头的预测文件不得进入任何格式的校准池。老文件（1.5 之前）缺 Format 头时按 `video` 处理
- `Rubric Version` 必填——将来 v3 时代回看 v2 预测，没有版本号就无法公平对比。版本号指**该 Format 对应 rubric** 的版本（`formats.<fmt>.rubric_version`）
- `预测时数据状态` 必填——明确声明 blind 是 immutable 承诺的前提
- `Script Path` 必填——指向打分所用的最终稿。video 是 `scripts/<id>.md`（pre-shoot 草稿）；article 是 `articles/<id>/draft.md`（或 draft.wechat-article.md）；xhs 是 `xhs/<id>/post.md`
- `Script Hash` 必填——video 在 cheat-shoot 时再 hash `videos/<id>/script.md`，不一致 → 复盘段加 integrity warning；article/xhs 无 shoot 环节，发布时由 cheat-publish 再 hash 终稿，起同样的完整性校验作用
- `Target Duration (s)` **仅 video**——state.typical_duration_seconds 派生。article/xhs 无此时长概念，写 `n/a`，篇幅约束看 `Actual Script Length` 的字数（article 4000-8000 字 / xhs 600-900 字）
- `Calibration Samples` + `Confidence` 必填——告诉读者这次预测有多可信。**Confidence 自动派生**自 calibration_samples（见 state-management.md），样本数按**本格式**计（`formats.<fmt>.calibration_samples`）
- `Prediction Basis` 必填——video：`pre_shoot` 为标准盲预测，`post_shoot_pre_publish` 为 v2 拍后改稿重判（仍未见数据，但软盲）；article/xhs 无拍摄环节，固定写 `pre_publish`（发布前盲预测）。article/xhs 目前**没有 v2 机制**——发布前大改终稿需要重判时，整份预测作废重建新文件（`_redo.md`），不 append v2 段
...
> 上例是 video 口径。article/xhs 的实绩指标与派生比率按 [state-management.md 实绩指标映射](state-management.md#实绩指标映射按格式) 填写（article=阅读/在看/赞阅比；xhs=曝光/赞藏/收藏率），桶的单位也随之换算（阅读数 / 赞藏量级）。
...
- **v2 触发条件（仅 video）**：cheat-shoot 检测拍摄稿与 `scripts/<id>.md` 的 line-diff ≥ 30%（[V2_TRIGGER_THRESHOLD](../skills/cheat-shoot/SKILL.md)），调用 `/cheat-predict — mode: v2 — prediction-file: <path>`。**article/xhs 没有 shoot 环节，v2 机制不适用**——发布前大改终稿时整份预测作废、新建 `_redo.md`，原文件保留
...
- `pre_shoot`（video v1 默认，标准盲预测）
- `post_shoot_pre_publish`（video v2，软盲预测——拍后改稿但发布前重判）
- `pre_publish`（article/xhs 固定值——无拍摄环节，发布前对终稿盲预测）
...
- diff `Script Hash` 与终稿的 hash → 不一致则在复盘段加 `**Script changed between predict and publish**` 警告。终稿路径按格式：video 是 `videos/<id>/script.md`（shoot 时落盘），article 是 `articles/<id>/draft*.md`，xhs 是 `xhs/<id>/post.md`（publish 时落盘）
