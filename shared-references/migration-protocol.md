...
```
| 1.1 | 1.2 | NO/YES | [1.1-to-1.2.md](../migrations/1.1-to-1.2.md) | 一句话描述 |
```
...
# 1.5 写法：formats 优先，顶层 legacy fallback（formats 不存在时老字段当 video 读）
shoots = state.get("formats", {}).get("video", {}).get("shoots", state.get("shoots", []))
...
**例外**：核心标识字段允许直接索引（如 `state["schema_version"]`）——缺失意味着 state 文件根本不合法，应该明确报错。注意 1.5 起 `rubric_version` 已按格式分列，顶层不再有该字段；要读 rubric 版本用 `state["formats"][state["default_format"]]["rubric_version"]`，直接索引顶层会 KeyError。
