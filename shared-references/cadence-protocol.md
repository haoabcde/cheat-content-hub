...
1. SessionStart hook 自动渲染状态报告（`hooks/session-start.sh` 是实际输出的正本）：
   - 📂 Channel 路径
   - 📦 默认格式
   - 🎬 视频 / 📝 公众号 / 📱 小红书 三行分格式状态（rubric 版本 / 校准样本 / 待复盘；video 行另含 Buffer 数）
   - 📦 Buffer 行（仅 video 显示颜色警戒；article / xhs 显示"非视频格式不使用拍摄队列"）
   - ⏰ 待复盘（到期项 + 最早到期日）
   - 🎯 候选 top 3
   - 📅 上次抓热点（N 天前）
   - 📈 校准样本 + Confidence 等级
   - ⚠️ 关键 to-do（buffer 警戒 / schema 不一致 / hook 未装等，按需出现）
2. 不主动开始任何动作——等用户决定
...
**Buffer = `state.formats.video.shoots` 数组长度** = 已拍但未发布的视频数。非 video 格式不使用拍摄 buffer。

`/cheat-shoot` 把视频加进 `state.formats.video.shoots`，`/cheat-publish` 移除——两个事件分开使 buffer 跟踪准确。legacy state 缺 `formats` 时可 fallback 读 `state.shoots`。
...
## 非视频格式的节奏跟踪（article / xhs）

article / xhs **不使用拍摄队列**——没有 `shoots`、没有 buffer。因此以下小节全部以 video 为前提，对非视频格式不适用：

- "Buffer 颜色对推荐的覆盖"表
- "节奏元规则"
- "标准化'今日工作流'模板"（情况 1-5）

非视频格式的断更兜底改用发布时间戳：读 `formats.<fmt>.last_published_at`，距上次发布超过 `target_publish_cadence_days × 2` 未发 → SessionStart / cheat-status 提示断更风险。`/cheat-publish` 登记发布时刷新该字段。
...
- 或验证某个**新 pattern**（用户项目 `script_patterns.md` 的 Pattern N）
...
├─ 拍 → cheat-recommend 只推当前 top 1 稳分 → 立即拍
...
| `/cheat-shoot` | 把 video folder 加 `formats.video.shoots`，buffer +1 |
| `/cheat-publish` | 从 `formats.video.shoots` 移除对应项，buffer -1 |
