...
#
# Removes the 20 cheat-on-content skills from Claude Code and/or Codex skill dirs.
#
...
# Uninstall is deliberately stricter than install. Install asks before replacing
# a colliding skill; uninstall must never delete a same-named skill that was
# installed by another package after the fact.
...
canonical_path() {
  ...
  parent="$(dirname -- "$path")"
  base="$(basename -- "$path")"
  (cd -P -- "$parent" 2>/dev/null && printf '%s/%s\n' "$PWD" "$base")
}

is_our_symlink() {
  local target="$1" source="$2" raw
  raw="$(readlink "$target")"
  if [[ "$raw" != /* ]]; then
    raw="$(dirname -- "$target")/$raw"
  fi
  [[ "$(canonical_path "$raw")" == "$(canonical_path "$source")" ]]
}

is_our_copy() {
  local target="$1" source="$2"
  # A copied install is safe to remove only while it still exactly matches this
  # source package. A modified or unrelated directory is user-owned.
  diff -qr -x .git "$source" "$target" >/dev/null 2>&1
}
  ...
  for s in "$@"; do
    ...
    if [[ -L "$target" ]]; then
      if is_our_symlink "$target" "$source"; then
        ...
        echo "  ⚠️  kept foreign symlink: $s"
      fi
    elif [[ -d "$target" ]]; then
      if is_our_copy "$target" "$source"; then
        rm -rf "$target"
        echo "  ✓ removed package copy: $s"
        REMOVED=$((REMOVED + 1))
      else
        echo "  ⚠️  kept modified or foreign directory: $s"
      fi
    else
