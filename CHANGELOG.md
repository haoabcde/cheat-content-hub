...
- Q5 文案补 content-guard 描述；Phase 5 下一步清单按格式分叉（articles//xhs/ 路径），补排版 / 配图 / aihot 发现项
...
**Format 字段断链（曾导致跨格式数据污染风险）**
- `shared-references/prediction-anatomy.md`：组件 1 header 补 `**Format**` 必填字段（与 prediction.template.md 对齐），是校准池按格式过滤的唯一依据；Target Duration / Script Path / Script Hash / Prediction Basis / v2 逐条加按格式注记（article/xhs 走 draft 终稿链、无 v2 机制、Basis 固定 `pre_publish`）
- `shared-references/bump-validation-protocol.md`：加"bump 只作用于单一格式 rubric"立场声明；校准池定义限定 `Format == 本次 bump 格式`；实绩排序改用按格式主指标；state 字段补 `formats.<fmt>.` 前缀
- `skills/cheat-bump`：新增 Phase A1 格式守卫（只能 bump `default_format` 的 rubric，非默认格式明确拒绝）；Phase 2 校准池按 Format 过滤；Phase 6 Re-scored 行只写同格式样本；Phase 7 / B4 全部改写 `formats.<fmt>.`（旧 schema 保留顶层写入）

**指标口径抽象层**
- `shared-references/state-management.md` 新增"实绩指标映射（按格式）"表：video=播放/赞播比、article=阅读/在看/赞阅比、xhs=赞藏/收藏率；bucket 单位按格式；跨格式 composite/bucket 不直接可比。全项目改为引用此表，不再各自硬编码
- `skills/cheat-retro`：Phase 4b 改稿 diff 加格式分支——article/xhs 用 prediction header 的 Script Hash 对比发布终稿（原路径硬编码 `videos/<id>/script.md`，对新格式是死路径）；粘数提示与 memo 模板按格式换指标词
- `skills/cheat-status`：L82/L115 改读 `formats.<fmt>.last_bump_at/last_bump_self_audited`（与 cheat-bump 同步改，防脱钩）
- `skills/cheat-recommend`：rubric_version 改读 `formats.<fmt>.`；补候选池 composite 的 rubric 归属约定（跨格式排序不直接可比）；shoots 读补 legacy fallback

**模板与文档对齐**
- `templates/rubric_notes.template.md`：Bucket 段删除已不存在的 `bucket_scheme` 字段引用，重写为自动派生算法；补"本文件对应 default_format"立场
- `templates/retro.template.md`：实绩段改三格式变体；`templates/status.template.md`：按 hook 实际输出重构（分格式总览 + Buffer + Confidence，删远古 mode 字段）；`templates/workflow.template.md`：主链加格式分叉、补 xhs 小节与 articles//xhs/ 目录；`templates/candidates.template.md`：composite 加格式标注约定；`templates/benchmark.template.md`、`templates/script_patterns.template.md`、`examples/script_patterns.example.md`：video-only 措辞标注适用范围或改格式中立
- `shared-references/cadence-protocol.md`：日级报告约定对齐 hook 实际输出；新增"非视频格式的节奏跟踪"小节；`shared-references/migration-protocol.md`：两处 1.5 前 flat 读取示例更正

**工具**
- `tools/workflow_audit.py`：新增 examples/<channel>/ 通道发现（与 session-start.sh 同规则），settings 检查在通道布局下同时接受仓库根配置——修复仓库根运行时的 state-missing / hook-settings-missing 双误报
- 新增 `tools/content_guard_test.sh`：content-guard hook 首个回归测试（6 case：放行/Rule 1/Rule 2/Rule 3/bypass），已接入 `tools/verify-package.sh`

### Fixed — upstream 健壮性修复找回（本地重写多格式版本时丢失，2026-09-18 合入）

本地在重写 multi-format（schema 1.5）时覆盖了官方同期修复，本次逐项找回：

- **hooks/session-start.sh**：补回官方 `parse_iso_epoch()` 跨平台时间戳解析（GNU `date -d` + BSD `date -j` 双 fallback），替换 3 处 BSD-only `date -j -f` 硬编码——此前该 hook 在 Linux 上日期计算全部失效
- **hooks/prediction-immutability.sh**：临时文件清理改回官方 `trap ... EXIT` 模式——本地只在两个正常退出点显式 `rm`，异常路径（set -e / 信号）会残留 tmpfile
- **hooks/log-event.sh**：补回 event_type 白名单校验（非 `tool_use|user_prompt|session_start|session_end` 一律记为 `unknown`），防止事件分类被脏数据污染
- **adapters/perf-data/xhs-explore/crawler.py**：补回 `__INITIAL_STATE__` 解析的尾分号清洗（`rstrip().rstrip(';')`）——公开页内联 JSON 以 `;` 结尾时 `json.loads` 直接失败
- **adapters/perf-data/douyin-session/crawler.py**：补回官方 `_pick()` None-aware fallback，替换 `or` 链——真值判断会把合法的 0 播放量当成缺失值，从 fallback dict 捏造非零数
- **两个 renderer.py**：截断的原始 JSON 补 `... (truncated)` 标记——此前截断处无标记，半截 JSON 看起来像完整数据
- **6 个 skill 的 `argument-hint` 全部加引号**（#69/#74 同方向）——裸 `[...]` 是 YAML flow-sequence，Copilot CLI ≥1.0.65 会静默丢弃整个 skill

### Added — upstream 同步内容补记（代码此前已同步，changelog 漏录）

- **xhs-explore adapter 公开页兜底 / 归档 / 账号汇总**（官方 PR #46）：`fetch_public_note()` 无登录解析公开页、评论兜底、`archive`/`summarize` 子命令、图片下载
- **受众画像 persona**（cheat-persona skill + audience.md）：与 rubric 平行的第二个派生物——"谁在看"
- **cheat-seed Phase 4.5 humanizer 自检 + draft 字幕格式修复**（官方 PR）：去 AI 味 pass + 段落版/字幕版格式分离
- **三个新 perf-data adapter**：bilibili-stat（#41）、linkedin-session（#51）、wechat-channels 视频号（#47，注意：作者声明未做真实登录态抓取验证）
- **cheat-init gitignore 凭证保护**（#48）：init 第一步创建/补全 .gitignore，挡住 `.auth*/` `.cheat-secrets.json`
- **douyin-session 运行时路径隐私修复**（#16）+ 自动文件夹去重（#58）
- **cheat-shoot DIFF_METRIC 口语化场景 v2 误触发修复**（#14）：char_levenshtein_normalized 默认度量
