...
>
> Schema 完整规范：skill 包内的 `shared-references/candidate-schema.md`。
...
- **composite (vN)**：当前 rubric 下的综合分（粗打分，**不是预测**）。必须标注打分所用格式 + rubric 版本（如 `composite (video v2): 9.18`）——跨格式的 composite 不直接可比（rubric 维度不同）
- **predicted bucket**：粗预测桶（粗略，仅用于排序）。桶单位按格式：video=播放数 / article=阅读数 / xhs=赞藏量级（口径见 skill 包内 `shared-references/state-management.md` 的"实绩指标映射（按格式）"表）
...
> 示例展示的是「视频分析」项目当前 candidates pool 的真实样本（已脱敏）；示例的维度串（ER/HP/QL/...）是 **video 格式的 rubric 维度**——article / xhs 用自己的 rubric 维度。
