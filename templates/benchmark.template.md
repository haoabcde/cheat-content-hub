...
>
> 你随时可以：
> - **追加**对标作品：`/cheat-learn-from --append <new-samples>`
...
| # | 作品标题 | 主指标 | 次指标 | 你的印象 | 原文路径 |
|---|---|---|---|---|---|
| 1 | [示例] 怎么停止期待 | 播放 71w | 点赞 2.4w / 评论 899 / 转发 1.8w | 高 | samples/[BENCHMARK-NAME]/ab61ed09/transcript.md |
| 2 | [示例] 老板废话 | 播放 39w | 点赞 1.2w / 评论 567 / 转发 7.9k | 中 | samples/[BENCHMARK-NAME]/5fe5d869/transcript.md |
| 3 | [示例] 谁问你了 | 播放 11w | 点赞 3.8k / 评论 198 / 转发 2.7k | 低 | samples/[BENCHMARK-NAME]/8b5627e6/transcript.md |

> **指标口径按格式**：主指标 / 次指标填什么见 skill 包内 `shared-references/state-management.md` 的"实绩指标映射（按格式）"表——video=播放 + 互动率（原文存 transcript.md）；article=阅读 + 在看（存全文 md）；xhs=赞藏（存全文 md）。上表示例是 video 口径。

> **印象档**：高 / 中 / 低 是你读完 / 看完这条作品后**主观**判断的"算不算这个账号的代表作"——不是数据驱动，是直觉判断。这个判断比数据更能告诉 Claude 你想做什么风格。
...
- 当当前格式的 `state.formats[format].calibration_samples >= 10` 时，cheat-status 会提示"你已有足够自己数据，benchmark 主要作 sanity check 不再主导"
