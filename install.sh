...
#                                      #  does NOT touch .cheat-state.json, .claude/settings.json, or user data)
...
if [[ "${1:-}" == "--reinstall-hooks" ]]; then
  ...
  for hook_script in prediction-immutability.sh session-start.sh log-event.sh content-guard.sh; do
  ...
  echo ""
  echo "Important: this only refreshed files under .cheat-hooks/."
  echo "It did NOT merge hooks/*.json into .claude/settings.json."
  echo "If this project was initialized before content-guard existed, run /cheat-init"
  echo "with hooks enabled again, or manually merge hooks/content-guard.json into settings."
  echo ""
