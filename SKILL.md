...
argument-hint: "[draft-path] [--format video|article|xhs]"
...
>
> 默认假设：**用户是从零开始的新人**。cold-start 期会标低 confidence，不强行制造精确感；随着 `formats.<format>.calibration_samples` 增长，预测自动变得更完整。
...
本系统在原有视频基础上，扩展支持三种内容格式：

| 格式 | 标识 | 产物目录 | rubric |
|---|---|---|---|
| 视频 | `video` | `videos/` | `starter-rubrics/opinion-video*.md` |
| 公众号长文 | `article` | `articles/` | `starter-rubrics/wechat-article*.md` |
| 小红书图文 | `xhs` | `xhs/` | `starter-rubrics/xhs-post*.md` |

**格式指定方式**：
1. 命令参数：`打分这篇 scripts/xxx.md --format=xhs`
2. 默认格式：`.cheat-state.json` 的 `default_format` 字段
3. `/cheat-init` 时选择

**硬性格式门槛**：除 `/cheat-init` 外，所有动作在读取 rubric、写 state、创建产物目录或选择 adapter 前，必须先解析出唯一 `format`。

解析顺序：显式 `--format` → prediction header 的 `Format` → 输入路径前缀（`videos/` / `articles/` / `xhs/`）→ `.cheat-state.json.default_format` → legacy `content_form` 推断。仍无法唯一确定时，必须询问用户，禁止默认乱选 `video` / `article` / `xhs`。

选定格式后，状态读写统一走 `formats.<format>`；legacy flat state 只作为老项目兼容 fallback，不作为新写入位置。

---

## 路由表（触发词 → 子 skill）

> **格式感知**：以下路由中，cheat-score / cheat-predict / cheat-retro / cheat-bump 会根据格式自动选择对应 rubric。cheat-shoot 仅对 video 格式有效。
...
> cheat-shoot 仅对 video 格式有效。对 article/xhs 触发"拍了"时，提示"该格式无需拍摄登记"。
...
**State detection**（首次接到非 init 触发词时执行）：
1. 检查用户当前目录是否有 `.cheat-state.json` → 没有 → 强制路由到 `/cheat-init`
2. 解析 `--format`、prediction header 或 `.cheat-state.json.default_format` → 得到当前格式
3. 读取 `formats.<format>.calibration_samples` → 派生 confidence 等级；不再写 `mode`
4. 再路由到目标 skill
...
- 「cheat-shoot 这篇公众号」 → 拒绝。cheat-shoot 仅用于 video 格式。改用"已发布"
- 「跳过格式选择直接打分」 → 拒绝。必须明确格式才能选择正确 rubric
...
│   └── wechat-editorial-workflow.md   # ✅ 公众号成稿前置流程：参考拆解/封面/配图/复制 HTML
...
│   ├── wechat-article.md              # ✅ 公众号长文（v0 未校准，传播驱动）
│   ├── wechat-article-zero.md         # ✅ 公众号 cold-start 版
│   ├── xhs-post.md                    # ✅ 小红书图文（v0 未校准，收藏驱动）
│   └── xhs-post-zero.md              # ✅ 小红书 cold-start 版
...
│   ├── gitignore.template             # ✅ 初始化第 0 步安全门槛
...
│   ├── content-guard.json             # ✅ 内容模板 / AI HOT 路由强制层
│   └── content-guard.sh               # ✅ content-guard 拦截脚本
├── tools/                             # 独立 CLI 脚本
│   ├── score-curve.py                 # ⬜ 预测精度收敛曲线
│   ├── diff_pct.py                    # ✅ cheat-shoot 改稿幅度计算
│   ├── diff_pct_test.sh               # ✅ diff_pct smoke test
│   ├── migrate_state.py                # ✅ schema 迁移链执行器（dry-run + 备份 + 原子写）
│   ├── workflow_audit.py              # ✅ 对抗性检查：草稿/预测/发布/复盘断链
│   ├── check_markdown_links.py         # ✅ 包内 Markdown 链接检查（跳过用户产物模板）
