...
#
# Usage:
#   bash run.sh <aweme_id> <artifact_folder> [<script_path>]
#
# Example:
#   bash run.sh 7234567890123456789 ~/my-channel/videos/2026-05-04_abc123_停止期待
#
# Output: writes report.md INTO the artifact_folder.
# Exit codes:
...
if [[ -z "$AWEME_ID" || -z "$ARTIFACT_FOLDER" ]]; then
  echo "Usage: bash run.sh <aweme_id> <artifact_folder> [<script_path>]" >&2
  exit 3
fi
...
PYTHON=""
PROJECT_ROOT="$( dirname "$( dirname "$( realpath "$ARTIFACT_FOLDER" )" )" )"
...
# Make sure artifact_folder exists
mkdir -p "$ARTIFACT_FOLDER"

# Resolve script path (optional — if given, copy to artifact_folder/script.txt for renderer)
SCRIPT_ARG=""
...
# Override output dir via env var so review.py writes to user's artifact directory.
export CHEAT_ARTIFACTS_DIR="$( dirname "$ARTIFACT_FOLDER" )"
export CHEAT_VIDEOS_DIR="$CHEAT_ARTIFACTS_DIR"  # backward compatibility for review.py

echo "[douyin-session] fetching aweme_id=$AWEME_ID into $ARTIFACT_FOLDER"
...
# review.py writes to CHEAT_ARTIFACTS_DIR/<auto-named-folder>/report.md.
# We need to find the just-written report.md and move it to our artifact_folder if names differ.
# (review.py uses video.title for folder name; ours uses <date>_<id>_<short>.)
LATEST_REPORT=$(find "$( dirname "$ARTIFACT_FOLDER" )" -name "report.md" -newer "$ARTIFACT_FOLDER" -type f 2>/dev/null | head -1)
if [[ -n "$LATEST_REPORT" && "$( dirname "$LATEST_REPORT" )" != "$ARTIFACT_FOLDER" ]]; then
  # Move the autonamed folder's report into our canonical folder
  cp "$LATEST_REPORT" "$ARTIFACT_FOLDER/report.md"
  # Optionally also copy script.txt if review.py wrote it
  AUTO_DIR=$( dirname "$LATEST_REPORT" )
  if [[ -f "$AUTO_DIR/script.txt" ]]; then
    cp "$AUTO_DIR/script.txt" "$ARTIFACT_FOLDER/script.txt"
  fi
  echo "[douyin-session] moved auto-named output to $ARTIFACT_FOLDER/"
fi

if [[ ! -f "$ARTIFACT_FOLDER/report.md" ]]; then
  echo "❌ report.md not produced — see review.py output above for details" >&2
  exit 3
fi

echo "✅ report.md written to $ARTIFACT_FOLDER/report.md"
exit 0
