...
def _pick(primary: dict, fallback: dict, key: str, default=0):
    """None-aware fallback: a real 0 in `primary` wins over the fallback dict.

    (Truthiness-based `or` chains would treat a legitimate 0 as missing and
    fabricate a non-zero value from the fallback.)"""
    v = primary.get(key)
    if v is not None:
        return v
    v = fallback.get(key)
