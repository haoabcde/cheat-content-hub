...
#
# Usage:
#   bash run.sh <note_id> <artifact_folder> [<script_path>]
#
# Example:
#   bash run.sh 66f1a2b3c4d5e6f700112233 ~/my-channel/xhs/2026-05-04_abc123_标题
#
...
if [[ -z "$NOTE_ID" || -z "$ARTIFACT_FOLDER" ]]; then
  echo "Usage: bash run.sh <note_id> <artifact_folder> [<script_path>]" >&2
  exit 3
fi
...
echo "[xhs-explore] fetching note_id=$NOTE_ID into $ARTIFACT_FOLDER"
...
# Move the just-written report into our canonical artifact_folder if names differ.
...
if [[ -n "$LATEST_REPORT" && "$( dirname "$LATEST_REPORT" )" != "$ARTIFACT_FOLDER" ]]; then
  ...
  echo "[xhs-explore] moved auto-named output to $ARTIFACT_FOLDER/"
fi
