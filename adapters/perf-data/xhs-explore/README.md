...
输出写到**你的内容项目** `xhs/<...>/report.md`（或 cheat-retro 传入的 artifact folder）。
...
万一小红书改版导致某项显示 0：打开 `xhs/<...>/report.md` 末尾的"galaxy 原始字段"JSON，
...
> 不在 `~/.claude/skills`（install.sh 只复制 19 个 skill，不复制 adapter）。
...
输出在当前目录的 artifact 目录下；cheat-retro 调用时通常是 `xhs/<日期>_<标题>/report.md`。archive 输出在 `<output_root>/<note_id>/`。
