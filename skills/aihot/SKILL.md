...
**每次成功调用 aihot API 并输出结果后，必须写标记文件**。这是 content-guard hook 的强制要求——hook 会检查 AI/tech 内容是否来自 aihot 而非训练数据。

```bash
# 创建 .cheat-cache 目录（如果不存在）
mkdir -p "${CLAUDE_PROJECT_DIR:-.}/.cheat-cache"

# 追加今天的日期到标记文件
echo "$(date +%Y-%m-%d)" >> "${CLAUDE_PROJECT_DIR:-.}/.cheat-cache/aihot-called.log"
```

标记文件格式：每行一个日期（YYYY-MM-DD）。content-guard hook 会检查今天的日期是否在文件中。

**不要跳过这一步**——没有标记文件，后续写 AI/tech 内容时会被 hook 阻断。
