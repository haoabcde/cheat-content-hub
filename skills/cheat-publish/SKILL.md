...
格式解析顺序：用户参数 `--format` → prediction header 的 `Format` → 输入路径前缀（`videos/` / `articles/` / `xhs/`）→ `.cheat-state.json.default_format` → legacy `content_form` → 询问用户。`format` 决定产物目录：video → `videos/`，article → `articles/`，xhs → `xhs/`。无法解析出唯一格式时必须停下来问，禁止默认按 video 发布。
...
**Artifact Folder**: videos/2026-05-04_a3f2c1d4_停止期待/
...
- video：到 cheat-publish 这一步，对应的 `videos/<id>/` 目录**应该已经由 cheat-shoot 创建**（含 script.md）。
  - 如 video folder 不存在 → 警告"你跳过了 cheat-shoot？建议先跑 cheat-shoot 把拍摄稿登记进 video folder 再发"，**询问用户是否跳过登记直接发**。
  - 用户确认跳过 → 自动建 `videos/<id>/` fallback，但不询问稿子一致性，标 `ad_hoc_publish: true`。
  - 用户不确认 → 让用户先跑 cheat-shoot 再回来 publish。
- article/xhs：**不要要求 cheat-shoot**。若 `articles/<id>/` 或 `xhs/<id>/` 不存在，直接创建对应 artifact folder，并提示用户可把终稿放到 `draft.md` / `post.md` 便于 retro diff。
...
```json
{
  ...
  "formats": {
    "<format>": {
      ...
      "last_published_artifact_folder": "<artifact_dir>/<...>/",
      "last_published_platform_id": "<aweme_id / BV 号 / note_id 等>",
      "pending_retros": [
        "predictions/<filename>"
      ],
      "shoots": [
        // 仅 video：移除 video_folder 匹配本次发布的项；buffer -1
      ]
    }
  }
}
...
**`shoots` 队列处理**（仅 video，buffer 跟踪关键）：
1. 读 `state.formats.video.shoots[]`；legacy state 缺 `formats` 时 fallback 读 `state.shoots[]`
2. 找 `video_folder == 本次发布的 artifact_folder` 的项 → 移除
3. 如果没找到 → 警告"buffer 队列里没有这条视频。是直接发布没经过 /cheat-shoot 吗？"——不阻塞，但提示用户下次走 /cheat-shoot 让 buffer 跟踪准确
4. article/xhs 不读写 `shoots`，也不输出 buffer 警告

`last_published_platform_id` 是 cheat-retro 调 adapter 时的输入——如 douyin-session 需要 aweme_id 直接抓数据。

`formats.<format>.pending_retros` 是待复盘列表——`cheat-status` 会基于这个列表 + RETRO_WINDOW_DAYS 显示"今天该复盘哪些"。legacy flat state 只读作兼容 fallback，新写入统一放到 `formats.<format>`。
...
Buffer 颜色由 [shared-references/cadence-protocol.md](../../shared-references/cadence-protocol.md) 派生，仅 video 格式启用。如本次发布让 buffer 跌入红色（断更风险）→ 高亮警告"今天必须再拍 ≥1 条"。article/xhs 输出"已加入待复盘"，不显示拍摄 buffer。

## Key Rules

1. **不动预测段**。即使是修复笔误，也不允许在 publish 时改预测段
2. **不抓数据**。publish 是登记动作，不是数据回收（那是 cheat-retro 的活）
3. **state 字段名固定**。`formats.<format>.pending_retros` / `formats.<format>.last_published_at` 是其他子 skill（特别是 cheat-status / cheat-retro）依赖的契约
