...
4. 把 video folder 加进 `formats.video.shoots` 队列，buffer +1
...
cheat-shoot 仅适用于 video 格式。

1. 读 `.cheat-state.json` 的 `default_format`
2. 如果用户指定了 `--format`，用用户指定的
3. 如果格式不是 `video` → **拒绝**，提示：
   > "cheat-shoot 仅用于视频格式。公众号和小红书不需要拍摄登记。
   > 如果你要登记文章发布，请用 '已发布 [URL]'。
   > 如果你要检查文章配图，请用 '找配图'。"
...
[Phase 3: 写 videos/<id>/script.md]
  ↓
[Phase 4: append formats.video.shoots]
  ↓
...
读 `.cheat-state.json`，检查 `formats.video.shoots[]` 是否已含此 id；legacy state 缺 `formats` 时 fallback 读 `shoots[]`：
...
4. **buffer 计算实时**——每次 shoot / publish 后立刻重算，`formats.video.shoots` 是真值
...
- 下游：`/cheat-publish` 发布时把对应项从 `formats.video.shoots` 移除
- `/cheat-status` 看板的 buffer 数字直接来自 `formats.video.shoots.length`
- `/cheat-recommend` 看 buffer 颜色调推荐策略
- SessionStart hook 看 buffer 颜色决定报告第一行

## `formats.video.shoots` 数据结构
