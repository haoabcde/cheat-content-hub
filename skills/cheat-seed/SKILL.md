...
**字数**：
- video：按 `DRAFT_LENGTH` 派生（基于 `typical_duration_seconds`）。
- article：按长文模板与当前 article starter rubric 写作。
- xhs：固定目标 600-900 字；标题 ≤20 个中文字符。

#### xhs 格式专用写稿规则

当当前格式是 `xhs`（用户参数 `--format=xhs`、prediction header 或 `.cheat-state.json.default_format`）时，不走下面的视频段落稿规则，改用小红书图文规则：

1. 先读 `starter-rubrics/xhs-post.md` 的“小红书特有注意事项”。
2. 写 3 个候选标题，每个 ≤20 个中文字符，并标注策略：数字 / 痛点 / 悬念 / 直接价值。
3. 正文 600-900 字，前 2 行必须抓注意力；不要用“今天给大家分享”这类寒暄开头。
4. 结构只选一种主结构：清单体 / 教程体 / 故事体 / 对比体 / 测评体。不要混成散文流水账。
5. 手机排版：每段 2-4 句，段间空行，emoji 做段落锚点，不用 markdown 标题层级。
6. 结尾 1-2 句给自然互动：收藏理由、评论问题或下一篇想看的方向。
7. 落盘可参考 `templates/xhs-post.template.md`；仍写入 `scripts/<YYYY-MM-DD>_<id>_<short-title>.md`，等用户改完后再 `cheat-predict --format=xhs`。

如果 xhs draft 命中以下任一情况，必须重写正文段后再给用户：
- 推荐标题 >20 个中文字符
- 正文 <600 或 >900 字
- 开头前 2 行只是寒暄 / 背景铺垫
- 出现 H2/H3 markdown 小标题
- 连续 6 行以上每行都很短，像字幕而不是图文笔记
