...
## Phase A1: 格式守卫（两条路径共用，先做）

bump 只作用于**单一格式**的 rubric。开始任何 Phase 之前必须锁定格式：

1. 用户参数 `--format=<video|article|xhs>` → 用它
2. 未指定 → 读 `.cheat-state.json` 的 `default_format`
3. 仍无法确定 → 停下来问，**禁止默认按 video bump**

**硬限制：只能 bump `default_format` 的 rubric。** 原因：`rubric_notes.md` / `rubric-memo.md` 是单文件，契约上始终对应 default_format（见 state-management.md）。如果用户指定的 `--format` ≠ `default_format` → 拒绝执行，并说明：

> "当前 rubric 文件对应默认格式 `<default_format>`，不能 bump `<指定格式>` 的 rubric。要进化它，先把 state 的 `default_format` 切到 `<指定格式>`（改 `.cheat-state.json` 顶层字段），再重新跑 /cheat-bump。"

锁定格式后，本 skill 全文中的"校准池""所有样本""实绩"一律指**该格式**的子集；指标口径（主指标 / 派生比率）按 [state-management.md 实绩指标映射](../../shared-references/state-management.md#实绩指标映射按格式) 解释（video=播放 / article=阅读+在看 / xhs=赞藏）。
...
| 检查 | 失败处理 |
|---|---|
| 本格式校准池样本数（`formats.<fmt>.calibration_samples`）vs 观察强度 | **Claude 判断**——按 READINESS_HEURISTIC：默认 ≥5 样本但允许特例（强反例 / 强模因）。如不满足默认，Claude 必须**显式说明**为什么仍然提议 bump（"虽然只 N=3 样本，但 X 这条出现 composite Y vs 实绩 Z，这是 W 倍偏差"），让用户审视 |
...
Glob `predictions/*.md` 中**Format 头等于 Phase A1 锁定格式**、且有完整复盘段的文件 → 校准池。缺 Format 头的 1.5 前老文件按 `video` 处理——锁定格式不是 video 时排除它们。**其他格式的 prediction 一律不进池、不打分、不改写**（三格式 rubric 维度与指标单位不同，混池会让 Phase 3 排序一致性失真）。
...
每个样本：
  ...
  actual_rank: 用本格式主指标实绩排序的 rank（video=播放 / article=阅读 / xhs=赞藏，见 state-management.md 实绩指标映射）
...
- mcp__llm-chat__chat 不可用 → 优雅降级到 `CROSS_MODEL_AUDIT=false`，state file 标 `formats.<fmt>.last_bump_self_audited: true`（旧 schema 写顶层 `last_bump_self_audited`）
...
- 顶部 metadata 更新：
  ...
  - `**Upgrade memos**: 见 rubric-memo.md`（指针，不复制 Memo 内容）
...
对**每个本格式校准样本**的 prediction 文件，**底部追加**（不动预测段、不动复盘段；**其他格式的 prediction 文件一个字都不碰**）：
...
写入 Phase A1 锁定格式的命名空间（schema 1.5）：
...
```json
{
  "formats": {
    "<fmt>": {
      ...
      "calibration_samples_at_last_bump": <该格式当前 calibration_samples>
    }
  }
}
```

旧 schema 项目（无 `formats` 对象）→ 写顶层同名字段（legacy 路径）。
**不要**触碰其他格式子对象的任何字段。
...
读 `predictions/*.md` 中 **Format 头等于锁定格式**、有主指标实绩的样本（video=actual_plays / article=actual_reads / xhs=actual_likes+actual_collects，见 state-management.md 实绩指标映射）。缺 Format 头的老文件按 video 处理。下文 `actual_plays` 为示例写法，实际按格式取对应主指标。
...
2. 更新 `.cheat-state.json` 的 `formats.<fmt>.baseline_plays`（旧 schema 写顶层 `baseline_plays`；字段名保留，语义按格式映射——article 是基线阅读量、xhs 是基线赞藏量级）。bucket scheme 不持久化——下次 cheat-predict 实时派生
...
- 「bump 非默认格式的 rubric」 → 拒绝。rubric_notes.md / rubric-memo.md 是单文件、对应 `default_format`。先切 `default_format` 再 bump（见 Phase A1）
- 「校准池里把那几篇其他格式的也一起重打了吧」 → 拒绝。跨格式混池让 rank 一致性失真（Phase 2 格式过滤是硬约束）
...
- 格式：Phase A1 锁定格式（= `default_format`），校准池 / 重打 / state 写入全部限定该格式
- 修改：
  - `rubric_notes.md`（结构性更新，**绝不**写真实视频名 / 实绩）
  - `rubric-memo.md`（**新**——append Memo 全文，含证据 + 派生证据）
  - **本格式**的 `predictions/*.md`（追加 Re-scored 行，不动预测段）
  - `.cheat-state.json` 的 `formats.<fmt>`（旧 schema 写顶层）
- 下游：下一篇同格式 `/cheat-predict` 自动按新 rubric_version 打分
