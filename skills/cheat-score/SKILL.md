...
- **RUBRIC_PATH = rubric_notes.md** — 当前 rubric 来源（对应 default_format）
- **OUTPUT_DETAIL = full** — full: 含每维度理由；compact: 仅分数表
- **FORMAT_RUBRICS** — 各格式的 rubric 映射：
  - `video` → `starter-rubrics/opinion-video.md` 或 `rubric_notes.md`
  - `article` → `starter-rubrics/wechat-article.md` 或 `rubric_notes.md`
  - `xhs` → `starter-rubrics/xhs-post.md` 或 `rubric_notes.md`

> 💡 调用时覆盖：`/cheat-score draft.md — OUTPUT_DETAIL: compact`
> 💡 指定格式：`/cheat-score draft.md — FORMAT: article`

## 格式检测

1. 用户传入 `--format=X` → 使用 X
2. 未指定 → 如 draft 路径能唯一指向 `articles/` 或 `xhs/`，用路径推断
3. 仍未确定 → 读 `.cheat-state.json` 的 `default_format`
4. `default_format` 不存在 → 读 `.cheat-state.json` 的 `content_form` 推断（兼容旧版）
5. 都不存在 → 询问用户

选定格式后，读取对应的 rubric：
- 如果项目根有 `rubric_notes.md` 且 `default_format` 匹配 → 用 `rubric_notes.md`
- 否则 → 用 `starter-rubrics/` 下对应格式的 rubric

**硬门槛**：无法解析出唯一格式时必须停下来问，禁止默认按 video rubric 打分。所有 state 读取优先用 `formats.<format>`，legacy flat 字段只做兼容 fallback。
