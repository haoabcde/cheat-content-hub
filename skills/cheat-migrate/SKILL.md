...
1. 输出 "→ [N/M] 应用 {file}..."
2. 读 `migrations/<file>` → 找到 `## HOW` 段；兼容 `## HOW (Claude steps for /cheat-migrate)` 这类带括号说明的标题
3. **先运行可执行迁移器**（默认 dry-run，用户确认后才加 `--apply`）：
   ```bash
   python3 <skill_repo>/tools/migrate_state.py <content-project> --package-root <skill_repo>
   # 用户确认后：同一命令追加 --apply
   ```
   它读取 `migrations/registry.md`，逐步备份并原子写 state；1.3 → 1.4 的 blind-channel 文件拆分也由它保守执行。若 1.4 state 无法从 `content_form` 推断格式，先向用户确认，再加 `--default-format video|article|xhs`。不要绕过它手写并行迁移规则。
4. 每步完成后：
