...
| `candidates.md` | 用户项目根 |
| `predictions/*.md` | 用于去重 |
| `.cheat-state.json` | 当前格式 rubric_version（`formats.<fmt>.rubric_version`，legacy fallback 顶层）+ default_format |

> **composite 的 rubric 归属**：候选池共享，但池内每条 composite 是用**某一格式**的 rubric 打的（candidates.md 约定标注，如 `composite (video v2): 9.18`）。当前格式的 rubric 与标注不一致时，跨格式排序**不直接可比**——推荐时在同格式标注的候选内排序；池内全是其他格式的分时，提示用户"这些候选是按 <格式> rubric 打的分，建议用当前格式 rubric 重打后再排序"，而不是静默混排。
...
读 `state.formats.video.shoots`（legacy fallback：顶层 `state.shoots`）+ `state.target_publish_cadence_days` 算 video buffer 颜色（[cadence-protocol.md](../../shared-references/cadence-protocol.md)）。非 video 默认不应用 buffer 覆盖：
