...
| 格式 | 主要指标 | 数据来源 |
|---|---|---|
| video | 播放量、点赞、评论、转发 | 手动粘贴 / douyin-session adapter |
| article | 阅读量、点赞、在看、评论、转发 | 手动粘贴 / 微信公众号后台 |
| xhs | 曝光、点赞、收藏、评论 | 手动粘贴 / 小红书后台 |

复盘时根据格式提示用户粘贴对应格式的指标。

产物报告写入对应格式的目录：
- video → `videos/<ID>/report.md`
- article → `articles/<ID>/report.md`
- xhs → `xhs/<ID>/report.md`

## Inputs

| 必填 | 来源 |
|---|---|
| `<prediction-file>` 或 `<artifact-folder>` | 用户参数；缺失则从 `.cheat-state.json` 的 `formats.<format>.pending_retros[0]` |
...
- **`videos/2026-05-04_<id>_<short>/` / `articles/.../` / `xhs/.../`** → 找对应的 prediction 文件（按 id 匹配）+ 把 report.md 写到该 artifact folder 里
- 缺省 → 从 `pending_retros[0]` 取最早的

### 格式解析（强制）

复盘前必须先得到唯一 `format`，顺序：用户参数 `--format` → prediction header 的 `Format` → artifact folder 前缀（`videos/` / `articles/` / `xhs/`）→ `.cheat-state.json.default_format` → legacy `content_form` 推断。仍无法唯一确定时必须询问用户，禁止默认按 video 复盘。

选定后，所有状态读写优先使用 `formats.<format>`；legacy flat state 只作为老项目 fallback。实绩观察只写 `rubric-memo.md`，不得写入 `rubric_notes.md`。
...
2. **识别有效预测段**：扫所有 `## 预测...` 段（可能含 `## 预测`、`## 预测 v1`、`## 预测 v2` 等）：
   ...
   - video 格式下，`formats.video.shoots` 对应项的 `v2_prediction_written` 应与"是否存在 v2 段"一致——不一致则警告（state 与文件脱节）
...
按 `state.data_collection` 字段分两条路径——抓回数据后**写到 artifact folder 的 `report.md`**（如果 prediction 关联 artifact folder），同时解析摘要 inline 到 prediction 的复盘段。

#### Path A：`DATA_SOURCE=manual`（候补方案）

- 询问用户："粘贴这条作品的当前数据"——指标按格式提示（见 [state-management.md 实绩指标映射](../../shared-references/state-management.md#实绩指标映射按格式)）：video 要播放 / 点赞 / 评论 / 转发 / 收藏；article 要阅读 / 在看 / 点赞 / 转发 / 收藏；xhs 要曝光 / 点赞 / 收藏 / 评论。顺序无所谓，能识别就行
- 用户粘 → 解析提取数字
...
- 把粘的原始数据写到 `<artifact_dir>/<...>/report.md`（如有 artifact folder）
...
|---|---|---|
| `douyin` | `adapters/perf-data/douyin-session/` | `bash <adapters-dir>/douyin-session/run.sh <aweme_id> <artifact_folder>` |
| `xhs` | `adapters/perf-data/xhs-explore/` | `bash <adapters-dir>/xhs-explore/run.sh <note_id> <artifact_folder>` |
| `linkedin` | `adapters/perf-data/linkedin-session/` | `bash <adapters-dir>/linkedin-session/run.sh <activity_id> <artifact_folder>` |
| `wechat_channels` | `adapters/perf-data/wechat-channels/` | `bash <adapters-dir>/wechat-channels/run.sh <post_id> <artifact_folder>` |
| `youtube` | `adapters/perf-data/youtube-data-api/`（待） | 调 YouTube Data API（需 API key） |
| `bilibili` | `adapters/perf-data/bilibili-stat/` | `bash <adapters-dir>/bilibili-stat/run.sh <bvid> <artifact_folder>` |
| 其他 | 无 adapter | 优雅降级到 Path A |

> `<adapters-dir>` = 克隆源码处的 `cheat-on-content/adapters/perf-data/`（install.sh **不**复制 adapter 到 ~/.claude/skills，只复制 19 个 skill）。定位：`find ~ -path '*/cheat-on-content/adapters/perf-data' -type d | head -1`。
...
- adapter 输出在 `<artifact_folder>/report.md`（adapter 的 renderer.py 已经按这个格式写）
...
不管 Path A 还是 B，最终：
- `<artifact_dir>/<...>/report.md` 含完整原始数据（数字 + top 评论）
...
Diff **预测时稿子** vs **发布时稿子**，找出**改动且对流量有明显影响**的部分。两条路径按格式：

- **video**：`scripts/<id>.md`（pre-shoot 草稿）vs `videos/<id>/script.md`（实际拍摄稿——cheat-shoot 时落盘），改稿状态读 `formats.video.shoots` 队列项的 `script_consistency`
- **article / xhs**：无 shoot 环节。用 prediction header 的 `Script Hash` 对比当前 `Script Path` 文件的 hash——不一致说明用户在预测后发布前改了稿。预测时版本从 git history 取（predictions 入库时稿子也应在 git 里），与现文件 diff
...
> **rubric 进化 ≠ 写作进化**——两者解耦：
> - rubric-memo.md 学的是"这次实绩暴露了哪些候选规律"，可以含样本名、数据、评论和链接
> - rubric_notes.md 只在后续 `/cheat-bump` 后接收已验证且抽象化的通用规则
> - script_patterns.md 学的是"什么写法真的能起作用"
> 可能有交叉（如 MS 维度与"互动钩子" pattern），但记录在两个文件里是因为**作用域不同**——rubric 改了影响所有未来打分，pattern 改了影响所有未来 draft。

如果发布时稿子 **缺失**（video：cheat-shoot 时用户标 `script_lost`；article/xhs：Script Path 文件被删）→ 跳过 4b，没法 diff。
- **video**：`script_consistency = "consistent"`（拍时没改稿）→ 4b 可以快速跳过细查；`"modified"` → **4b 是核心**，重点学这次改动 → 流量影响
- **article/xhs**：hash 一致 → 快速跳过；hash 不一致 → **4b 是核心**
...
读 `.cheat-state.json` 当前格式的 `formats.<format>.consecutive_directional_errors` 字段，按本次复盘判定向更新：
...
- 状态字段更新：`formats.<format>.calibration_samples` +1（这是 cheat-status 显示进度的关键）
- `formats.<format>.pending_retros`：剔除本条
