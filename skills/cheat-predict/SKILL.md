...
> **格式检测**：与 cheat-score 相同。选定格式后：
> - 读取对应格式的 rubric（见 cheat-score 的 FORMAT_RUBRICS）
> - prediction 文件统一写入 `predictions/` 目录（所有格式共享）
> - prediction header 的 `Format` 字段标记内容格式（枚举：video/article/xhs，不是 content_form）
> - state 读写统一使用 `formats.<format>`；legacy flat state 只作兼容 fallback
>
> 无法解析出唯一格式时必须询问用户，禁止默认套用 video rubric。
...
- **`Format`**：`video` / `article` / `xhs`——注意是格式枚举，**不是** content_form（不写 `xhs-post`、`wechat-article`）。校准池按格式过滤靠它（见 [prediction-anatomy.md](../../shared-references/prediction-anatomy.md) 组件 1）
...
```json
{
  "in_progress_session": {
    ...
    "artifact_folder": "<videos|articles|xhs>/YYYY-MM-DD_<id>_<short>/",
...
`artifact_folder` 为 null 表示用户跑的是裸 .md 文件，尚未建对应产物目录。video 的旧字段 `video_folder` 可作为 legacy fallback 读取，但新写入用 `artifact_folder`。

`in_progress_session` 在 `cheat-publish` 触发时清除。如果用户预测后从未 publish（弃稿），下次 `/cheat-init` 或 `/cheat-status` 检测到陈旧 in_progress 会询问是否清理。

`formats.<format>.last_prediction_self_scored`：
...
- `formats.<format>.last_self_scored_at` 跟随更新；下次同格式 `cheat-predict` 走 sub-agent → 这两个字段一起被重置
