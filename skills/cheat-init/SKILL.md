...
argument-hint: "[--format video|article|xhs] [--form opinion-video|wechat-article|xhs-post|long-essay|short-text|podcast]"
...
[Phase 2.5: 对标账号 — 强烈建议（cold-start 必须问，已发用户可选）]
  ↓
[Phase 3: 创建脚手架（含 scripts/ + predictions/ + videos/ + articles/ + xhs/ + samples/ 空目录 + 模板文件含 benchmark.md）]
  ↓
...
> "你的内容更接近哪一种？
> a) **视频**（抖音 / B 站 / YouTube / 视频号）— 走 `videos/`，可用 cheat-shoot 管 buffer
> b) **公众号 / 长文**（公众号 / Substack / Medium）— 走 `articles/`
> c) **小红书图文**（小红书笔记 / 图文种草）— 走 `xhs/`
> d) **短文 / thread**（X / 微博 / 即刻）— 先按小红书图文工作流走，后续可 bump
> e) **播客 / 长视频** — 先按视频工作流走，后续可 bump
> f) **其他 / 混合** — 先选最接近的产物目录，rubric 后续校准"

记录到 `default_format`、`formats.<format>.content_form`、`formats.<format>.artifact_dir`、`formats.<format>.rubric_form_mismatch`。

**Q1 → state 映射**（**必须存 enum 值，不是字母**）：

| 用户答 | `default_format` | `artifact_dir` | `content_form` | `rubric_form_mismatch` |
|---|---|---|---|---|
| a | `"video"` | `"videos"` | `"opinion-video"` | `false` |
| b | `"article"` | `"articles"` | `"wechat-article"` | `false` |
| c | `"xhs"` | `"xhs"` | `"xhs-post"` | `false` |
| d | `"xhs"` | `"xhs"` | `"short-text"` | `true` |
| e | `"video"` | `"videos"` | `"podcast"` | `true` |
| f | 追问 `video/article/xhs` | 对应目录 | `"other"` 或 `"mixed"` | `true` |

`rubric_form_mismatch=true` 时，cheat-status 持续提示"你的形态可能需要 bump 调权重"。
...
如 manual 模式：
> "你大概发过多少条内容？给个范围就行（比如 '5-10 条' / '20+ 条'），
...
0. **`.gitignore`（安全门槛，必须第一步创建）**
   ```
   "先创建 / 补全 .gitignore，把账号凭证挡在版本控制外——这是第一件事。
    .auth/、.auth-xhs/、.auth-wechat-channels/ 存的是抖音 / 小红书 / 视频号登录态；
    .cheat-secrets.json 存 API key / cookie；
    .cheat-cache/ 存本地运行日志、aihot 标记和 adapter debug。

    注意：predictions/、scripts/、videos/、articles/、xhs/、samples/ 不忽略。
    它们是预测档案、产物目录和校准循环的一部分，原则 #1/#3 依赖 git history。"
   ```
   - 如果 `<user-repo>/.gitignore` 不存在：复制 `cheat-on-content/templates/gitignore.template` 创建
   - 如果已存在：**不覆盖**，逐行检查并追加缺失项，至少确保这些行存在：
     - `.auth/`
     - `.auth-xhs/`
     ...
     - `.cheat-cache/`
     - `.DS_Store`
   - 即使用户项目还没 `git init`，也照常创建 / 补全 `.gitignore`
   - 如果用户此前可能误加过凭证，提示执行：`git rm -r --cached .auth .auth-xhs .auth-wechat-channels .cheat-secrets.json .cheat-cache`
...
1. **`.cheat-state.json`**
   ...
   ```json
   {
     "schema_version": "1.5",
     "skill_version": "1.0.0",
     "default_format": "<查 Q1 映射表，写 \"video\"/\"article\"/\"xhs\">",
     ...
     "formats": {
       "video": {
         ...
         "rubric_form_mismatch": <Q1 非 video starter 时 true，否则 false>,
         "artifact_dir": "videos",
         "baseline_plays": null,
         "calibration_samples": <default_format=video 时按 Q2 写入，否则 0>,
       ...
       "article": {
         ...
         "rubric_form_mismatch": <default_format=article 且 Q1 不是公众号 starter 时 true，否则 false>,
         "artifact_dir": "articles",
         "baseline_plays": null,
         "calibration_samples": <default_format=article 时按 Q2 写入，否则 0>,
         "calibration_samples_at_last_bump": 0,
         "consecutive_directional_errors": [],
         "pending_retros": [],
         "last_bump_at": null,
       ...
       "xhs": {
         ...
         "rubric_form_mismatch": <default_format=xhs 且 Q1 不是小红书 starter 时 true，否则 false>,
         "artifact_dir": "xhs",
         "baseline_plays": null,
         "calibration_samples": <default_format=xhs 时按 Q2 写入，否则 0>,
         "calibration_samples_at_last_bump": 0,
         "consecutive_directional_errors": [],
         "pending_retros": [],
         "last_bump_at": null,
...
4. **六个目录**：`scripts/` + `predictions/` + `videos/` + `articles/` + `xhs/` + `samples/`（都加 `.gitkeep`）
   ```
   "正在创建六个目录：
    
    scripts/      — 拍前的草稿（cheat-seed 写或你写）
    predictions/  — immutable 预测日志（hook 保护）
    videos/       — 视频产物目录（cheat-shoot 创建子目录）
    articles/     — 公众号 / 长文产物目录
    xhs/          — 小红书图文产物目录
    samples/      — 对标账号视频 / 转录（cheat-learn-from 创建子目录）
    
    scripts/、predictions/ 和对应产物目录用同一组 <date>_<id>_<short> 命名相互关联。
...
6. **如果 Q5=是 → 安装 hooks**
   ...
   - merge 进 `hooks/content-guard.json` 的 `hooks.PreToolUse`
   - 复制 `prediction-immutability.sh` + `session-start.sh` + `log-event.sh` + `content-guard.sh` 到 `.cheat-hooks/`，chmod +x
...
1. **建 artifact folder**：`<artifact_dir>/<date>_<id>_<short>/`
   - `<artifact_dir>` 来自当前格式：video → `videos/`，article → `articles/`，xhs → `xhs/`
   - `<date>` = 内容实际发布日
   - `<id>` = 12 位 hash，对 (title + 平台 ID) 做 sha256
   - `<short>` = 标题前 3-8 字
2. **写 report.md**：从 adapter 抓回的数据（播放 / 点赞 / 评论 / 转发 / top 评论）填入
3. **询问用户原稿**："这条内容「{标题}」你保留了原稿吗？"
   - 是 → 用户提供 → video 存为 `videos/<id>/script.md`；article 存为 `articles/<id>/draft.md`；xhs 存为 `xhs/<id>/post.md`
   - 否 → 标 `script_lost`（仍建 artifact folder，只是原稿文件缺失）
...
- 输出汇总："已 import N 条历史。最近一条 X w 表现，中位数 Y w，已建 N 个 artifact folder + reconstructed predictions"
...
6. content-guard hook 验证：向 `.cheat-hooks/content-guard.sh` 喂一个缺 metadata 的 `articles/_test.md` Write payload → 应 exit 1 并提示使用 `templates/draft.template.md`
...
6. **import 失败不阻塞**：Q2=b 但 adapter 装失败 / 抓取失败 → 优雅降级到"标 calibration_samples 估值，不导入历史 artifact folder"
...
- `cheat-status` 读 `.cheat-state.json` 的 `formats.<format>.calibration_samples` 字段决定显示哪个 confidence 等级
- 如 Q2=b 走了 import → 历史 reconstructed predictions 进 `predictions/` 和对应产物目录（`videos/` / `articles/` / `xhs/`），但**不**计入 calibration_samples（不是真校准样本）
...
| `default_format` | Phase 3 | Q1 → 查映射表换 `"video"` / `"article"` / `"xhs"` |
| `formats.<format>.rubric_version` | Phase 3 | "v0" |
| `formats.<format>.content_form` | Phase 3 | Q1 → 查映射表换 enum 值（**不是字母**） |
| `formats.<format>.artifact_dir` | Phase 3 | Q1 → `videos` / `articles` / `xhs` |
| `typical_duration_seconds` | Phase 3 | Q1.5 派生 |
| `target_publish_cadence_days` | Phase 3 | Q1.6 派生 |
| `formats.<format>.rubric_form_mismatch` | Phase 3 | Q1 映射表派生 |
...
| `formats.<format>.baseline_plays` | Phase 3.5（如 import 成功） | import 数据中位数；否则 null |
| `formats.<format>.calibration_samples` | Phase 3 / Phase 3.5 | Q2=a→0；Q2=b→Q2.3 估值或 import 数 |
...
| `last_trends_run_at` | Phase 3 | `null` |
| `last_trends_added_count` | Phase 3 | `0` |
| `formats.<format>.last_prediction_self_scored` | Phase 3 | `false` |
| `formats.<format>.last_self_scored_at` | Phase 3 | `null` |
